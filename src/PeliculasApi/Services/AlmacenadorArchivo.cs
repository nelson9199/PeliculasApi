...
{
    public class AlmacenadorArchivo : IAlmacenadorArchivos
    {
        ...
        public async Task<string> GuardarArchivo(byte[] contenido, string extension, string contenedor, string contentType)
        {
            ...
            await contenedorRef.SetPermissionsAsync(new BlobContainerPermissions() { PublicAccess = BlobContainerPublicAccessType.Blob });
